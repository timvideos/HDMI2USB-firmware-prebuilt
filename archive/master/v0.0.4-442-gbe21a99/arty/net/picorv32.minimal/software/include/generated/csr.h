...
static inline unsigned int ethmac_sram_writer_length_read(void) {
	...
	r <<= 8;
	r |= csr_readl(0xe0007808);
	r <<= 8;
	r |= csr_readl(0xe000780c);
	r <<= 8;
	r |= csr_readl(0xe0007810);
	return r;
}
...
static inline unsigned short int ethmac_sram_reader_length_read(void) {
	unsigned short int r = csr_readl(0xe0007840);
	r <<= 8;
	r |= csr_readl(0xe0007844);
	return r;
}
static inline void ethmac_sram_reader_length_write(unsigned short int value) {
	csr_writel(value >> 8, 0xe0007840);
	csr_writel(value, 0xe0007844);
}
...
static inline unsigned char ethmac_preamble_crc_read(void) {
	unsigned char r = csr_readl(0xe0007854);
	return r;
}
...
static inline unsigned int ethmac_preamble_errors_read(void) {
	unsigned int r = csr_readl(0xe0007858);
	r <<= 8;
	r |= csr_readl(0xe000785c);
	r <<= 8;
	r |= csr_readl(0xe0007860);
	r <<= 8;
	r |= csr_readl(0xe0007864);
	return r;
}
...
static inline unsigned int ethmac_crc_errors_read(void) {
	unsigned int r = csr_readl(0xe0007868);
	r <<= 8;
	r |= csr_readl(0xe000786c);
	r <<= 8;
	r |= csr_readl(0xe0007870);
	r <<= 8;
	r |= csr_readl(0xe0007874);
	return r;
}
...
#define CONFIG_CPU_VARIANT "PICORV32"
